...
## Real-model conversion audit (separate from performance runs)

Enable **both** `DGPP_MIMO_FP8_KV=1 DGPP_MIMO_FP8_KV_AUDIT=1` on each TP rank. Other audit values fail; enabling audit with BF16 fails. This option is off by default. Enabled models allocate 256 bytes per base layer (12,288 bytes for 48 layers) and launch one additional reduction kernel after each append. Default-off append kernels and cache layout are unchanged; there is no BF16 cache shadow. Keep audit off for latency/throughput measurements.

On clean model teardown, each rank logs one `MiMo FP8 KV audit` record per base layer and type K/V containing `conversions`, `clipped`, `nonfinite`, `changed`, `error_squared`, and `reference_squared`. Native BF16 MTP and independent draft caches are excluded. Logs also identify audit enablement at startup and the memory plan includes its small allocation. Abrupt termination, a CUDA failure, or bypassing model destruction can prevent readout. Low-level probes may instead pass a device array of two `MimoFp8AuditStats` to `mimo_qkv_append`, synchronize, and download it; layer probes can use `set_fp8_audit`.

Samples count **executed valid append elements**, including warmup, cache-only execution, speculative/rejected writes, and graph replays. Padding and invalid positions follow append's exact skip rules. Snapshot copies/restores are not conversions and do not increment counters; neither request reset nor speculative rollback rewinds the audit. Counts are not unique or committed-token counts. The audit recomputes append's exact BF16 post-RoPE K and BF16 post-.707 V from unchanged fused inputs, then applies the same unit-scale E4M3 encode/decode. It does not read overwritten cache entries, so duplicate-position warmup does not mix one row's source with another row's stored value. This is a conversion diagnostic, not an independent cache-integrity check; normal append's caller-owned uniqueness contract is unchanged.

`clipped` counts finite inputs with absolute value above E4M3's largest finite magnitude, 448 (range exceedance, including values that would round to the endpoint). `nonfinite` counts NaN or infinity at the post-transform input. Only finite inputs contribute to `changed`, squared error, and reference squared norm. Compute conversion RMS as `sqrt(error_squared / (conversions - nonfinite))`, and relative L2 as `sqrt(error_squared / reference_squared)` when denominators are nonzero. Aggregate by summing counts/squared sums across desired layers/ranks before taking roots. FP64 block reductions issue six atomics per CTA; final summation order may vary slightly. These measurements provide clipping/rounding evidence, not model-quality equivalence.

The focused `mimo_fp8_audit_counts_transformed_inputs_padding_and_graph_replay` GPU probe checks disabled counters, finite clipping, NaN/infinity, independent CPU-codebook counts/error/norm from production BF16-transformed inputs, global/SWA positions, mapped padding/invalid rows, and cumulative graph replay. Run `mimo_fp8_test` plus `compute-sanitizer --tool memcheck --error-exitcode 1 ./mimo_fp8_test` on idle hardware before a separate real-model audit pass. No Spark execution is performed by this implementation task.
